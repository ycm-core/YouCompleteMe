...
class DiagnosticInterface( object ):
  ...
  def _UpdateSigns( self ):
    placed_signs = vimsupport.GetSignsInBuffer( self._bufnr )

    for line, diags in iteritems( self._line_to_diags ):
      if not diags:
        continue

      # We always go for the first diagnostic on line because diagnostics are
      # sorted by errors in priority.
      ...
      try:
        placed_signs.remove( sign )
      except ValueError:
        vimsupport.PlaceSign( sign )
        self._next_sign_id += 1

    # Remaining signs are not part of the diagnostics or there is already a sign
    # on their lines (Vim can only display one sign by line). Remove them.

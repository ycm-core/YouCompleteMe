...
class YouCompleteMe( object ):
  ...
  def GetCompletionsUserMayHaveCompleted( self ):
    ...
    if 'user_data' in completed_item and completed_item[ 'user_data' ] != '':
      # Vim supports user_data, so we actually know the _exact_ element that
      # was selected, having put its index in the user_data field.
  ...
  def _OnCompleteDone_Java( self ):
    ...
    # If we have user_data in completions, then we would only ever return max. 1
    # completion here. However, if we had to guess, it is possible that we
    # matched multiple compleion items (e.g. for overloads, or similar classes
    # in multiple packages). In any case, rather than prompting the user and
    # disturbing her workflow, we just apply the first one. This might be wrong,
    # but the solution is to use a (very) new version of Vim which supports
    # user_data on completion items

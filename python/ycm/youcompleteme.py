...
class YouCompleteMe( object ):
  ...
  def _AddUltiSnipsDataIfNeeded( self, extra_data ):
    try:
      snippets = vimsupport.GetVariableValue(
          'UltiSnips#SnippetsInCurrentScope( 1 )' )
    except vim.error:
      return
    # UltiSnips#SnippetsInCurrentScope() returns a dictionary where each key is
    # the name of snippet trigger word and its corresponding value its
    # description.
    extra_data[ 'ultisnips_snippets' ] = [
      { 'trigger': trigger,
        'description': description }
      for trigger, description in iteritems( snippets )
    ]

...
def AutocommandEventsIgnored( events = 'all' ):
  ...
  events. |events| is a comma separated list of events to ignore. By default,
  all events are ignored."""
  old_eventignore = vim.options[ 'eventignore' ]
  vim.options[ 'eventignore' ] += ToBytes( events )
  try:
    yield
  finally:
...
def SwitchWindow( window ):
  """Move to a different window. This function should be called under the
  CurrentWindow context if you are going to switch back to the previous
  window."""

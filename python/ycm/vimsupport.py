...
def ReplaceChunk( start, end, replacement_text, line_delta, char_delta,
  ...
  if replacement_lines_count == 1:
    ...
    # The replacement text ends on a different line so we need to substract the
    # original end column (and not the shifted one if the replaced text is on
    # one line).
    char_delta = end_replacement_column - ( end[ 'column_num' ] - 1 )

...
def OpenLocationList( focus = False, autoclose = False ):
  ...
  if autoclose:
    # Ensure only the location list window is closed.
    vim.command( 'augroup ycmlocation' )
    vim.command( 'autocmd! * <buffer>' )
    vim.command( 'autocmd WinLeave <buffer> q' )
...
def OpenQuickFixList( focus = False, autoclose = False ):
  ...
  if autoclose:
    # Ensure only the quickfix window is closed.
    vim.command( 'augroup ycmquickfix' )
    vim.command( 'autocmd! * <buffer>' )
    vim.command( 'autocmd WinLeave <buffer> q' )
    vim.command( 'augroup END' )

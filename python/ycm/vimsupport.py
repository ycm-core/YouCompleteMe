...
def ReplaceChunksInBuffer( chunks, vim_buffer ):
  """Apply changes in |chunks| to the buffer-like object |buffer|. Return the
  locations for that buffer."""

  # We apply the chunks from the bottom of the buffer to its top so that we
  # don't need to adjust the position of chunks due to the text changes. This

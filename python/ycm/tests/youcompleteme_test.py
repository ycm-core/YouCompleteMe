...
def RunNotifyUserIfServerCrashed( ycm, test, post_vim_message ):
  ...
  ycm._logger.error.assert_has_calls(
    [ call( message ) for message in test[ 'expected_logs' ] ] )
...
@YouCompleteMeInstance()
def YouCompleteMe_DebugInfo_test( ycm ):
...
@YouCompleteMeInstance()
def YouCompleteMe_OnVimLeave_ClientLogfileIsRemoved_test( ycm ):
    ycm.OnVimLeave()

    client_logfile = ycm._client_logfile
    assert_that( is_not( os.path.isfile( client_logfile ) ),

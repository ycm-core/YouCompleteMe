...
def YouCompleteMe_OnPeriodicTick_DontRetry_test( ycm,
  ...
  # not)
  with MockVimBuffers( [ current_buffer ], [ current_buffer ], ( 1, 1 ) ) as b:
    with patch.dict( ycm._message_poll_requests,
        { 'ycmtest': MockAsyncServerResponseInProgress( b ) } ) as mock_future:
      poll_again = ycm.OnPeriodicTick()
      mock_future[ 'ycmtest' ]._response_future.done.assert_called()
      mock_future[ 'ycmtest' ]._response_future.result.assert_not_called()
      assert_that( poll_again, equal_to( True ) )

  # Poll again, but return a response (telling us to stop polling)
  with MockVimBuffers( [ current_buffer ], [ current_buffer ], ( 1, 1 ) ) as b:
    with patch.dict( ycm._message_poll_requests,
        { 'ycmtest': MockAsyncServerResponseDone( False ) } ) as mock_future:
      poll_again = ycm.OnPeriodicTick()
      mock_future[ 'ycmtest' ]._response_future.done.assert_called()
      mock_future[ 'ycmtest' ]._response_future.result.assert_called()
...
def YouCompleteMe_OnPeriodicTick_Exception_test( ycm,
  ...
  with MockVimBuffers( [ current_buffer ], [ current_buffer ], ( 1, 1 ) ) as b:
    mock_response = MockAsyncServerResponseException(
        b, RuntimeError( 'test' ) )
    with patch.dict( ycm._message_poll_requests,
        { 'ycmtest': mock_response } ) as mock_future:
      assert_that( ycm.OnPeriodicTick(), equal_to( False ) )
      mock_future[ 'ycmtest' ]._response_future.done.assert_called()
      mock_future[ 'ycmtest' ]._response_future.result.assert_called()
      post_data_to_handler_async.assert_not_called()
...
def YouCompleteMe_OnPeriodicTick_ValidResponse_test( ycm,
  ...
  with MockVimBuffers( [ current_buffer ], [ current_buffer ], ( 1, 1 ) ) as b:
    with patch.dict( ycm._message_poll_requests,
        { 'ycmtest': MockAsyncServerResponseDone( b, [] ) } ) as mock_future:
      assert_that( ycm.OnPeriodicTick(), equal_to( True ) )

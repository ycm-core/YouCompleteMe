  ...
  try:
    with patch( 'ycm.tests.test_utils.server_python_interpreter',
      ...
      post_vim_message.assert_called_once_with(
        ...
        "Path in 'g:ycm_server_python_interpreter' does not point "
        "to a valid Python 2.6+ or 3.3+. Revise the option "
        "and start again the server with ':YcmRestartServer'." )
  ...
  try:
    with patch( 'ycmd.utils.ReadFile', side_effect = IOError ):
      ...
      post_vim_message.assert_called_once_with(
        ...
        "interpreter path and start again the server with "
        "':YcmRestartServer'." )

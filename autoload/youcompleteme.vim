...
function! s:SetUpKeyMappings()
  ...
  " keys?
  inoremap <expr> <BS> <SID>OnBackspaceKey()
endfunction
...
function! s:OnTextChangedInsertMode()
  ...
  " Call previous completion to avoid flickers.
  call s:Complete()
...
function! s:IdentifierFinishedOperations()
  ...
  let s:force_semantic = 0
  " TODO: should we really reset completions here? By doing so, the completion
  " menu is automatically closed after finishing an identifier but also when
  " deleting the last character of an identifier which cause a flicker after a
...
function! s:InvokeSemanticCompletion()
  ...
  exec s:python_command "ycm_state.CreateCompletionRequest( True )"
...
function! s:PollCompletion( ... )
  if !s:Pyeval( 'ycm_state.IsCompletionDone()' )

...
function! youcompleteme#Enable()
  ...
  let s:timers.poll_server_ready.id = timer_start(
        \ s:timers.poll_server_ready.wait_milliseconds,
...
function! s:OnFileReadyToParse( ... )
  ...
  if force_parsing || b:changedtick != get( b:, 'ycm_changedtick', -1 )
    ...
    call timer_stop( s:timers.poll_file_parse_response.id )
    let s:timers.poll_file_parse_response.id = timer_start(
          \ s:timers.poll_file_parse_response.wait_milliseconds,
...
function! s:RestartServer()
  ...
  call timer_stop( s:timers.on_server_ready.id )
  let s:timers.on_server_ready.id = timer_start(
        \ s:timers.on_server_ready.wait_milliseconds,
        \ function( 's:OnServerReady' ) )
endfunction

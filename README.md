...
NOTE: Previously, YCM used [rls][] for ruest completion. This is no longer
supported, as the Rust community has decided on rust-analyzer as the future of
rust tooling.
...
`config_sections` dictionary which maps section to a distionay of config
...
YCM no longer uses RLS for rust, and these optionas are therefore no longer
supported.

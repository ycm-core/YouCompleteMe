...
will be used as it is but if is an executable name it will be searched through
...
YCM will use the first `python` executable it founds in the PATH to run
[jedi][]. This means that if you are in a virtualenv and you start vim in that
directory, the first `python` that YCM will find will be the one in the
virtualenv, so [jedi][] will be able to provide completions for every package
you have in the virtualenv.
...
### I can't complete python packages in a virtualenv.

This means that the Python used to run [JediHTTP][] is not the Python of the
virtualenv you're in. To resolve this you either set `g:ycm_python_binary_path`
to the absolute path of the Python binary in your virtualenv or since
virtualenv will put that Python executable first in your PATH when the
virtualenv is active then if you set `g:ycm_python_binary_path` to just
`'python'` it will be found as the first Python and used to run [JediHTTP][].

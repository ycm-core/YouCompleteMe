...
this flag does **not** install **clangd**. You need to specificy it manually by
...
3.  *Complete this step ONLY if you care about semantic completion support for
    ...
    **Download the latest version of `libclang`**. Clang is an open-source
    ...
    tell YCM where to find clangd binary. Please note that YCM is desinged to
...
doubt we recommend using [libclang]-based engine. Here is a quick comparision of
the two completer engines:
...
-   **Project wide indexing**: Clangd has both dynamic and static index support.
    ...
    currently editing, wherea static index contains project-wide symbol
    information. This symbol information is used by CodeCompletion, GoTo and
    FindReferences requests. Whereas libclang is limited to current TU.
...
-   **Rename**: Clangd can perform semantic rename operations on current file,
    whereas libclang doesn’t support such functionality.
-   **Code Completion**: Clangd can perform code completions at a lower latency
    than libclang also it has information about all the symbols on your project
    so it can suggest items outside your current TU and also provides
...
mentioned before, pass `--clang-completer` when in doubt. Since [clangd][]-based
completer is still experimental.
...
sets the fallback path in case clangd binary in third party doesn't exist.
...
This option controls the command line arguments passed to clangd binary, it
